...
def main():
    ...
    for i, column in enumerate(columns):
        print(
            format(column, "<15"),
            format(LAMMPS_data[i].ensemble_avg(), "+8.4E"),
            " +/- ",
...
def parse_commandline_opts():
    ...
    op.add_option(
        "-f", "--file", help="Specify the file containing LAMMPS time-series data"
    )
    op.add_option(
        "-b", "--blocks", help="Specify the number of blocks for estimating error"
    )
    op.add_option(
        "-m", "--minsteps", help="Specify the number of timesteps for equilibration"
    )
